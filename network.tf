...
resource "oci_core_subnet" "mysql_subnet" {
  count                      = var.use_existing_networking ? 0 : 1
  cidr_block                 = lookup(var.network_cidrs, "MYSQL-SUBNET-REGIONAL-CIDR")
  compartment_id             = var.compartment_ocid
  ...
  route_table_id             = oci_core_route_table.oke_private_route_table[0].id
  dhcp_options_id            = oci_core_virtual_network.oke_vcn[0].default_dhcp_options_id
  security_list_ids          = [oci_core_security_list.mysql_security_list[0].id]
  defined_tags               = local.defined_tags
}
...
resource "oci_core_security_list" "oke_nodes_security_list" {
  ...
  egress_security_rules {
    description      = "Allow MySQL access to MySQL DB System"
    destination      = lookup(var.network_cidrs, "MYSQL-SUBNET-REGIONAL-CIDR")
    destination_type = "CIDR_BLOCK"
    protocol         = local.tcp_protocol_number
    stateless        = false
...
resource "oci_core_security_list" "mysql_security_list" {
  count          = var.use_existing_networking ? 0 : 1
  compartment_id = var.compartment_ocid
  display_name   = "mysql-seclist"
  vcn_id         = oci_core_virtual_network.oke_vcn[0].id

  # Ingress Rules - Allow MySQL access from Kubernetes nodes
  ingress_security_rules {
    description = "Allow MySQL access from Kubernetes worker nodes"
    source      = lookup(var.network_cidrs, "SUBNET-REGIONAL-CIDR")
    source_type = "CIDR_BLOCK"
    protocol    = local.tcp_protocol_number
    stateless   = false
  ...
  ingress_security_rules {
    description = "Allow MySQL access from Kubernetes pods"
    source      = lookup(var.network_cidrs, "PODS-CIDR")
    source_type = "CIDR_BLOCK"
    protocol    = local.tcp_protocol_number
    stateless   = false
